...
const translateErrorMessage = (message) => {
  ...
  if (
    messageStr.includes("user_not_existed") ||
  ...
  if (
    messageStr.includes("bad credentials") ||
  ...
  if (
    messageStr.includes("uncategorized") ||
  ...
  try {
    ...
    if (response.status === 200 && response.data) {
      ...
      if (accessToken) {
        ...
        if (refreshToken) {
          console.log(
            "✅ Saving refreshToken to localStorage:",
  ...
  try {
    ...
    if (errorData) {
      if (errorData.message) {
        ...
        if (
          backendMessage === "USER_EXISTED" ||
          backendMessage.includes("already exists")
        ) {
          errorMessage = "Email đã được đăng ký";
        } else if (
          backendMessage === "PASSWORD_MISMATCH" ||
          backendMessage.includes("do not match")
        ) {
          errorMessage = "Mật khẩu xác nhận không khớp";
        } else if (
          backendMessage === "EMAIL_INVALID" ||
          backendMessage.includes("Email format is invalid")
        ) {
          errorMessage = "Email không hợp lệ";
        } else if (
          backendMessage === "EMAIL_REQUIRED" ||
          backendMessage.includes("Email is required")
        ) {
          errorMessage = "Vui lòng nhập email";
        } else if (
          backendMessage === "PASSWORD_REQUIRED" ||
          backendMessage.includes("Password is required")
        ) {
          errorMessage = "Vui lòng nhập mật khẩu";
        } else if (
          backendMessage === "CONFIRM_PASSWORD_REQUIRED" ||
          backendMessage.includes("Confirm password is required")
        ) {
          errorMessage = "Vui lòng nhập xác nhận mật khẩu";
        } else if (
          backendMessage === "FULLNAME_REQUIRED" ||
          backendMessage.includes("Full name is required")
        ) {
          errorMessage = "Vui lòng nhập họ tên";
        } else if (
          backendMessage === "PHONE_REQUIRED" ||
          backendMessage.includes("Phone number is required")
        ) {
          errorMessage = "Vui lòng nhập số điện thoại";
        } else if (
          backendMessage === "PHONE_INVALID" ||
          backendMessage.includes("Phone number format is invalid")
        ) {
